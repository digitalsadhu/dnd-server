    ...
    command: state => (options, player) => {
        ...
        for (let i in availableQuests) {
            ...
            if (player.questTracker.canStart(quest)) {
                say(
                    player,
                    `[<b><yellow>!</yellow></b>] - ${displayIndex}. ${
                        quest.config.title
                    }`
                );
                ...
                say(
                    player,
                    `[<b><yellow>${symbol}</yellow></b>] - ${displayIndex}. ${
                        quest.config.title
                    }`
                );
            }
...
subcommands.add({
    ...
    command: state => (options, player) => {
        ...
        for (let i in active) {
            ...
            B.at(
                player,
                '<b><yellow>' + (parseInt(i, 10) + 1) + '</yellow></b>: '
            );
            say(
                player,
                B.progress(60, progress.percent, 'yellow') +
                    ` ${progress.percent}%`
            );
            say(
                player,
            ...
            say(
                ...
                B.indent(
                    B.wrap(
                        `<b><yellow>${quest.config.description}</yellow></b>`,
                        78
                    ),
                    2
                )
            ...
            if (quest.config.rewards.length) {
                ...
                say(
                    player,
                    '<b><yellow>' + B.center(80, 'Rewards') + '</yellow></b>'
                );
                say(
                    player,
                    '<b><yellow>' + B.center(80, '-------') + '</yellow></b>'
                );
...
subcommands.add({
    ...
    command: state => (options, player) => {
        ...
        if (
            quest.config.npc &&
            ![...player.room.npcs].find(
                npc => npc.entityReference === quest.config.npc
            )
        ) {
            const npc = state.MobFactory.getDefinition(quest.config.npc);
            return say(
                player,
                `The questor [${npc.name}] is not in this room.`
            );
        }
