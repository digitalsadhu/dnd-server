    ...
    run: state =>
        function(args, player, target) {
            ...
            effect.on('effectDeactivated', _ => {
                Broadcast.sayAt(
                    player,
                    `<red><b>${target.name}</b> stops bleeding.</red>`
                );
            });

            Broadcast.sayAt(
                player,
                `<red>With a vicious attack you open a deep wound in <bold>${
                    target.name
                }</bold>!</red>`
            );
            Broadcast.sayAtExcept(
                player.room,
                `<red>${player.name} viciously rends ${target.name}.</red>`,
                [target, player]
            );
            Broadcast.sayAt(
                target,
                `<red>${player.name} viciously rends you!</red>`
            );
            target.addEffect(effect);
        },

    info: player => {
        return `Tear a deep wound in your target's flesh dealing <bold>${damagePercent}%</bold> weapon damage over <bold>${duration /
            1000}</bold> seconds.`;
    },
};
