...
subcommands.add({
    ...
    command: state => (args, player) => {
        ...
        say(
            player,
            "<b><green>You created a group, invite players with '<white>group invite <name></white>'</green></b>"
        );
    },
});
...
subcommands.add({
    ...
    command: state => (args, player) => {
        if (!player.party) {
            return say(
                player,
                "You don't have a group, create one with '<b>group create</b>'."
            );
        }
        ...
        say(
            ...
            `<b><green>${
                player.name
            } invited you to join their group. Join/decline with '<white>group join/decline ${
                player.name
            }</white>'</green></b>`
        );
        say(
...
subcommands.add({
    ...
    command: state => (args, player) => {
        ...
        if (!args || args !== 'sure') {
            return say(
                player,
                `<b><green>You have to confirm disbanding your group with '<white>group disband sure</white>'</green></b>`
            );
        }
...
subcommands.add({
    ...
    command: state => (args, player) => {
        ...
        say(
            target.party,
            `<b><green>${player.name} joined the group.</green></b>`
        );
...
subcommands.add({
    ...
    command: state => (args, player) => {
        ...
        say(
            player,
            `<b><green>You decline to join ${target.name}'s group.</green></b>`
        );
        say(
            target,
            `<b><green>${player.name} declined to join your group.</green></b>`
        );
