...
module.exports = {
    ...
    command: state => (args, player) => {
        ...
        const parts = args
            .split(' ')
            .filter(arg => !arg.match(/in/) && !arg.match(/into/));
        ...
        if (toContainer.closed) {
            return B.sayAt(
                player,
                `${ItemUtil.display(toContainer)} is closed.`
            );
        }
        ...
        B.sayAt(
            player,
            `<green>You put </green>${ItemUtil.display(
                item
            )}<green> into </green>${ItemUtil.display(
                toContainer
            )}<green>.</green>`
        );
