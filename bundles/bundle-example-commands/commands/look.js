...
const humanize = sec => {
    return require('humanize-duration')(sec, { round: true });
};
...
function getCompass(player) {
    ...
    const directionsAvailable = room.exits.map(exit =>
        exitMap.get(exit.direction)
    );
...
function lookRoom(state, player) {
    ...
    if (player.room.coordinates) {
        B.sayAt(
            player,
            '<yellow><b>' + sprintf('%-65s', room.title) + '</b></yellow>'
        );
        ...
        B.sayAt(
            player,
            B.line(65, ' ') + '<yellow><b>' + line3 + '</b></yellow>'
        );
    }
    ...
    room.items.forEach(item => {
        if (item.hasBehavior('resource')) {
            B.sayAt(
                player,
                `[${ItemUtil.qualityColorize(item, 'Resource')}] <magenta>${
                    item.roomDesc
                }</magenta>`
            );
        } else {
            B.sayAt(
                player,
                `[${ItemUtil.qualityColorize(item, 'Item')}] <magenta>${
                    item.roomDesc
                }</magenta>`
            );
        }
    ...
    room.npcs.forEach(npc => {
        ...
        if (npc.quests) {
            ...
            hasNewQuest = quests.find(quest =>
                player.questTracker.canStart(quest)
            );
            hasReadyQuest = quests.find(quest => {
                return (
                    player.questTracker.isActive(quest.entityReference) &&
                    player.questTracker.get(quest.entityReference).getProgress()
                        .percent >= 100
                );
            });
            ...
            if (hasNewQuest || hasActiveQuest || hasReadyQuest) {
                ...
                questString += hasActiveQuest
                    ? '[<b><yellow>%</yellow></b>]'
                    : '';
                questString += hasReadyQuest
                    ? '[<b><yellow>?</yellow></b>]'
                    : '';
...
function lookEntity(state, player, args) {
    ...
    if (entity.timeUntilDecay) {
        B.sayAt(
            player,
            `You estimate that ${entity.name} will rot away in ${humanize(
                entity.timeUntilDecay
            )}.`
        );
    }
    ...
    if (entity instanceof Item) {
        switch (entity.type) {
            ...
            case ItemType.ARMOR:
                return B.sayAt(
                    player,
                    ItemUtil.renderItem(state, entity, player)
                );
...
function getCombatantsDisplay(entity) {
    const combatantsList = [...entity.combatants.values()].map(
