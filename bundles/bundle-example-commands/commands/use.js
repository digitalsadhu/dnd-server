...
module.exports = {
    ...
    command: state => (args, player) => {
        ...
        if (usable.spell) {
            ...
            if (!useSpell) {
                Logger.error(
                    ...
                    } has invalid usable configuration.`
                );
                return say("You can't use that.");
            }
            ...
            try {
                ...
                if (e instanceof SkillErrors.CooldownError) {
                    return say(
                        `${useSpell.name} is on cooldown. ${humanize(
                            e.effect.remaining
                        )} remaining.`
                    );
                }
        ...
        if (usable.effect) {
            ...
            let useEffect = state.EffectFactory.create(
                usable.effect,
                effectConfig,
                effectState
            );
        ...
        if (usable.destroyOnDepleted && usable.charges <= 0) {
            say(
                `You used up all the magic in ${ItemUtil.display(
                    item
