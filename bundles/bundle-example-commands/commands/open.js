...
module.exports = {
    ...
    usage:
        '[open/close/lock/unlock] <item> / [open/close/lock/unlock] <door direction>/ [open/close/lock/unlock] <door direction>',
    ...
    switch (action) {
        ...
        case 'unlock': {
            ...
            if (door.lockedBy) {
                ...
                return B.sayAt(
                    player,
                    `The door can only be unlocked with ${keyItem.name}.`
                );
            }
    ...
    switch (action) {
        case 'open': {
            ...
            return B.sayAt(
                ...
                `${ItemUtil.display(
                    item
                )} is already open, you can't open it any farther.`
            );
        }
        ...
        case 'lock': {
            ...
            if (!item.lockedBy) {
                return B.sayAt(
                    player,
                    `You can't lock ${ItemUtil.display(item)}.`
                );
            }
            ...
            return B.sayAt(
                player,
                "The item is locked and you don't have the key."
            );
        }
        ...
        case 'unlock': {
            if (!item.locked) {
                return B.sayAt(
                    player,
                    `${ItemUtil.display(item)} isn't locked...`
                );
            }
            ...
            if (!item.closed) {
                return B.sayAt(
                    player,
                    `${ItemUtil.display(item)} isn't closed...`
                );
            }
            ...
            if (item.lockedBy) {
                ...
                if (playerKey) {
                    B.sayAt(
                        player,
                        `*click* You unlock ${ItemUtil.display(
                            item
                        )} with ${ItemUtil.display(playerKey)}.`
                    );
