...
module.exports = {
    ...
    command: state => (args, player) => {
        ...
        if (target === player) {
            return B.sayAt(
                player,
                `<green>You move ${ItemUtil.display(
                    targetItem
                )} from one hand to the other. That was productive.</green>`
            );
        }
        ...
        B.sayAt(
            player,
            `<green>You give <white>${target.name}</white>: ${ItemUtil.display(
                targetItem
            )}.</green>`
        );
        if (!target.isNpc) {
            B.sayAt(
                target,
                `<green>${player.name} gives you: ${ItemUtil.display(
                    targetItem
                )}.</green>`
            );
        }
