    ...
    listeners: {
        ...
        move: state =>
            function(movementCommand) {
                ...
                const door =
                    oldRoom.getDoor(nextRoom) || nextRoom.getDoor(oldRoom);
        ...
        commandQueued: state =>
            function(commandIndex) {
                ...
                const ttr = sprintf(
                    '%.1f',
                    this.commandQueue.getTimeTilRun(commandIndex)
                );
                B.sayAt(
                    this,
                    `<bold><yellow>Executing</yellow> '<white>${
                        command.label
                    }</white>' <yellow>in</yellow> <white>${ttr}</white> <yellow>seconds.</yellow>`
                );
            },
        ...
        updateTick: state =>
            function() {
                if (
                    this.commandQueue.hasPending &&
                    this.commandQueue.lagRemaining <= 0
                ) {
                    B.sayAt(this);
                ...
                const maxIdleTime =
                    Math.abs(Config.get('maxIdleTime')) * 60000 || Infinity;

                if (timeSinceLastCommand > maxIdleTime && !this.isInCombat()) {
                    this.save(() => {
                        B.sayAt(
                            this,
                            `You were kicked for being idle for more than ${maxIdleTime /
                                60000} minutes!`
                        );
                        B.sayAtExcept(
                            this.room,
                            `${this.name} disappears.`,
                            this
                        );
        ...
        experience: state =>
            function(amount) {
                B.sayAt(
                    this,
                    `<blue>You gained <bold>${amount}</bold> experience!</blue>`
                );
