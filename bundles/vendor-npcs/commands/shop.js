...
subcommands.add({
    ...
    command: state => (vendor, args, player) => {
        ...
        if (args) {
            ...
            if (!item) {
                return tell(
                    "I don't carry that item and no, I won't check in back."
                );
            }
            ...
            B.sayAt(
                player,
                `Cost: <b><white>[${friendlyCurrencyName(
                    vendorItem.currency
                )}]</white></b> x ${vendorItem.cost}`
            );
            return;
        }
        ...
        for (const [, itemCategory] of Object.entries(ItemType)) {
            ...
            B.sayAt(
                player,
                '.' + B.center(78, category.title, 'yellow', '-') + '.'
            );
            for (const item of category.items) {
                ...
                B.sayAt(
                    player,
                    '<yellow>|</yellow> ' +
                        ItemUtil.qualityColorize(
                            item,
                            sprintf('%-48s', `[${item.name}]`)
                        ) +
                        sprintf(
                            ' <yellow>|</yellow> <b>%-26s</b>',
                            B.center(
                                26,
                                friendlyCurrencyName(vendorItem.currency) +
                                    ' x ' +
                                    vendorItem.cost
                            )
                        ) +
...
subcommands.add({
    ...
    command: state => (vendor, args, player) => {
        ...
        say(
            player,
            `<green>You spend <b><white>${
                vendorItem.cost
            } ${friendlyCurrencyName(
                vendorItem.currency
            )}</white></b> to purchase ${ItemUtil.display(item)}.</green>`
        );
        player.save();
    },
});
...
subcommands.add({
    ...
    command: state => (vendor, args, player) => {
        ...
        if (
            !['poor', 'common'].includes(item.metadata.quality || 'common') &&
            confirm !== 'sure'
        ) {
            return say(
                player,
                "To sell higher quality items use '<b>sell <item> sure</b>'."
            );
        }
        ...
        player.setMeta(
            currencyKey,
            (player.getMeta(currencyKey) || 0) + sellable.value
        );

        say(
            player,
            `<green>You sell ${ItemUtil.display(item)} for <b><white>${
                sellable.value
            } ${friendlyCurrencyName(sellable.currency)}</white></b>.</green>`
        );
...
subcommands.add({
    ...
    command: state => (vendor, args, player) => {
        ...
        tell(
            `I could give you <b><white>${
                sellable.value
            } ${friendlyCurrencyName(
                sellable.currency
            )}</white></b> for ${ItemUtil.display(targetItem)}.</green>`
        );
    },
});
...
module.exports = {
    ...
    command: state => (args, player, arg0) => {
        ...
        const vendor = Array.from(player.room.npcs).find(npc =>
            npc.hasBehavior('vendor')
        );
        ...
        if (!subcommand) {
            return say(
                player,
                "Not a valid shop command. See '<b>help shops</b>'"
            );
        }
...
function genTell(state, vendor, player) {
    return message => {
        state.ChannelManager.get('tell').send(
            state,
            vendor,
            player.name + ' ' + message
        );
...
function friendlyCurrencyName(currency) {
    return currency.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase());
}
