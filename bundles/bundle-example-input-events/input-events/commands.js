            ...
            try {
                ...
                switch (result.type) {
                    ...
                    case CommandType.COMMAND: {
                        const {
                            requiredRole = PlayerRoles.PLAYER,
                        } = result.command;
                ...
                switch (true) {
                    case error instanceof InvalidCommandError:
                        ...
                        const roomCommands = player.room.getBehavior(
                            'commands'
                        );
                        const [commandName, ...args] = data.split(' ');
                        if (
                            roomCommands &&
                            roomCommands.includes(commandName)
                        ) {
                            player.room.emit(
                                'command',
                                player,
                            ...
                            B.sayAt(player, 'Huh?');
                            Logger.warn(
                                `WARNING: Player tried non-existent command '${data}'`
                            );
                        }
                        break;
