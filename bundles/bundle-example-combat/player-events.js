...
module.exports = {
    listeners: {
         ...
         */
        hit: state =>
            function(damage, target, finalAmount) {
                ...
                if (this.equipment.has('wield')) {
                    this.equipment
                        .get('wield')
                        .emit('hit', damage, target, finalAmount);
                }
        ...
        heal: state =>
            function(heal, target) {
                ...
                if (target !== this) {
                    ...
                    buf += `<b> ${
                        target.name
                    }</b> for <b><green>${finalAmount}</green></b> ${
                        heal.attribute
                    }.`;
                    B.sayAt(this, buf);
                }
                ...
                for (const member of this.party) {
                    ...
                    buf += ` <b>${target.name}</b>`;
                    buf += ` for <b><green>${finalAmount}</green></b> ${
                        heal.attribute
                    }.`;
                    B.sayAt(member, buf);
                }
            ...
            function(damage, finalAmount) {
                ...
                if (this.party) {
                    ...
                    for (const member of this.party) {
                        ...
                        buf += ` hit <b>${
                            this.name
                        }</b> for <b><red>${finalAmount}</red></b> damage`;
                        B.sayAt(member, buf);
                    }
                }
        ...
        healed: state =>
            function(heal, finalAmount) {
                ...
                if (heal.attribute === 'health') {
                    ...
                    buf = `${attacker}${source} restores <b>${finalAmount}</b> ${
                        heal.attribute
                    }.`;
                }
                B.sayAt(this, buf);
                ...
                for (const member of this.party) {
                    ...
                    let buf = `${attacker}${source} heals ${
                        this.name
                    } for <b><red>${finalAmount}</red></b>.`;
                    B.sayAt(member, buf);
                }
            },
        ...
        killed: state => {
            ...
            return function(killer) {
                ...
                const othersDeathMessage = killer
                    ? `<b><red>${
                          this.name
                      } collapses to the ground, dead at the hands of ${
                          killer.name
                      }.</b></red>`
                    : `<b><red>${
                          this.name
                      } collapses to the ground, dead</b></red>`;

                B.sayAtExcept(
                    this.room,
                    othersDeathMessage,
                    killer ? [killer, this] : this
                );

                if (this.party) {
                    B.sayAt(
                        this.party,
                        `<b><green>${this.name} was killed!</green></b>`
                    );
                }

                this.setAttributeToMax('health');

                let home = state.RoomManager.getRoom(
                    this.getMeta('waypoint.home')
                );
                if (!home) {
                ...
                this.moveTo(home, _ => {
                    ...
                    B.sayAt(
                        this,
                        `<red>You lose <b>${lostExp}</b> experience!</red>`
                    );
        ...
        deathblow: state =>
            function(target, skipParty) {
                ...
                if (target && !this.isNpc) {
                    B.sayAt(
                        this,
                        `<b><red>You killed ${target.name}!</red></b>`
                    );
                }
...
function promptBuilder(promptee) {
    ...
    const formatProgressBar = (name, progress, entity) => {
        ...
        return `<b>${name}${pad}</b>: ${progress} <b>${entity.getAttribute(
            'health'
        )}/${entity.getMaxAttribute('health')}</b>`;
    };
    ...
    for (const target of promptee.combatants) {
        let currentPerc = Math.floor(
            (target.getAttribute('health') / target.getMaxAttribute('health')) *
                100
        );
