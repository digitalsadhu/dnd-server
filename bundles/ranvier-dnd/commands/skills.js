...
module.exports = srcPath => {
    ...
    return {
        ...
        command: state => (args, player) => {
            ...
            for (const [level, abilities] of Object.entries(
                player.playerClass.abilityTable
            )) {
                ...
                for (let skillId of abilities.skills) {
                    let skill = state.SkillManager.get(v.slugify(skillId));

                    if (!skill) {
                        Logger.error(
                            `Invalid skill in ability table: ${
                ...
                for (let spellId of abilities.spells) {
                    let spell = state.SpellManager.get(v.slugify(spellId));

                    if (!spell) {
                        Logger.error(
                            `Invalid spell in ability table: ${
                                player.playerClass.name
                            }:${level}:${spellId}`
                        );
                        continue;
