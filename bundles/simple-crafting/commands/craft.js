        ...
        if (!itemNumber) {
            ...
            return category.items.forEach((categoryEntry, index) => {
                say(
                    player,
                    sprintf('%2d) ', index + 1) +
                        ItemUtil.display(categoryEntry.item)
                );
            });
        }
...
subcommands.add({
    ...
    command: state => (args, player) => {
        ...
        const craftingCategories = getCraftingCategories(state);
        const isInvalidCraftingCategory = isInvalidSelection(
            craftingCategories
        );
        ...
        for (const [resource, recipeRequirement] of Object.entries(
            item.recipe
        )) {
            ...
            if (playerResource < recipeRequirement) {
                return say(
                    player,
                    `You don't have enough resources. 'craft list ${args}' to see recipe. You need ${recipeRequirement -
                        playerResource} more ${resource}.`
                );
            }
        }
        ...
        for (const [resource, amount] of Object.entries(item.recipe)) {
            ...
            say(
                player,
                `<green>You spend ${amount} x ${ItemUtil.display(
                    resItem
                )}.</green>`
            );
        }
        ...
        say(
            player,
            `<b><green>You create: ${ItemUtil.display(item.item)}.</green></b>`
        );
        player.save();
    },
});
...
module.exports = {
    ...
    command: state => (args, player) => {
        ...
        if (!subcommand) {
            return say(
                player,
                'Invalid command. Use craft list or craft create.'
            );
        }
